...
**[`docs/产出样张集.md`](docs/产出样张集.md)** —— 既是样张集也是使用指南：12 份真实实跑产出，每份都配触发词（说 `跑早報` 出什么、说 `掃熱點` 出什么），顶部还有一张触发速查表。含跨源打架、数据稀薄、缺数据诚实标注三类边界案例。
...
单价为逐次实测读数（非估算）：早报 6-7 点、研报榜 1 点、研报解读 1 点/标的、温度计 1-2 点/标的、信号汇总 2 点、扫热点 1 点、速报/财报速读 2 点、标的速查 2 点。`news()` 全免费，`signal()` 一次 fanout 四类仍只计 1 点。

三种运营强度的月耗（**已含 30-40% 余量**，覆盖重试与临时查询）：

| 强度 | 周耗 | 月耗 | Basic 1,000/月 | 建议 |
|---|---|---|---|---|
| 轻量 | ~80 | ~350 | 用掉 35% | Basic 充裕 |
| 标准 | ~130 | ~550 | 用掉 55% | **Basic 即可** |
| 重度（财报季） | ~200 | ~850 | 用掉 85% | 贴顶，建议 Pro |

真实锚点：2026-07-22~23 两天把六个模块全部跑通（含回归测试、两份财报速读、信号汇总与若干次失败重试），共消耗 **58 点**。详细单价拆解见 [`skills-community/README.md`](skills-community/README.md)。
